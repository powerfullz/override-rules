...
var convert = (() => {
  ...
  var SPEEDTEST_URL = "http://www.gstatic.com/generate_204";
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      lowCostNodes.length > 0 || regexFilter2 ? buildGroupByType({
        name: PROXY_GROUPS.LOW_COST,
        icon: `${CDN_URL}/gh/Koolson/Qure@master/IconSet/Color/Lab.png`,
        groupType: groupType2,
        nodeSource: !regexFilter2 ? { proxies: lowCostNodes.map((node) => node.name).filter(isNotNull) } : { "include-all": true, filter: LOW_COST_NODE_MATCHER.pattern }
      }) : null,
      ...countryNames.map((country) => {
        const meta = countriesMeta[country];
        if (!meta) return null;
        const nodeSource = regexFilter2 ? {
          "include-all": true,
          filter: meta.pattern
  ...
  function buildRules({ quicEnabled: quicEnabled2 }) {
  ...
  function buildDnsConfig({ mode, ipv6Enabled: ipv6Enabled2, fakeIpFilter }) {
    const config = {
      enable: true,
      ipv6: ipv6Enabled2,
      "prefer-h3": true,
  ...
  function buildDns({ fakeIPEnabled: fakeIPEnabled2, ipv6Enabled: ipv6Enabled2 }) {
    if (fakeIPEnabled2) {
      return buildDnsConfig({ mode: "fake-ip", ipv6Enabled: ipv6Enabled2, fakeIpFilter: FAKE_IP_FILTER });
    }
    return buildDnsConfig({ mode: "redir-host", ipv6Enabled: ipv6Enabled2 });
  }

  // src/tun.ts
  function buildTunConfig(tunEnabled2) {
    return {
      enable: tunEnabled2,
      stack: "gvisor",
      device: "mihomo",
  ...
  function buildBaseLists({
    ...
    const lowCost = lowCostNodes.length > 0 || regexFilter2;
    ...
    const frontProxySelector = buildList(
      suffixedCountryNames,
      "DIRECT",
      !regexFilter2 && nonLandingNodes.map((node) => node.name).filter(Boolean)
    );
    return {
      defaultProxies,
      defaultProxiesDirect,
