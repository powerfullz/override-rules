...
(() => {
  ...
  // src/constants.ts
  var NODE_SUFFIX, CDN_URL, SPEEDTEST_URL, PROXY_GROUPS, LOW_COST_NODE_MATCHER, LANDING_NODE_MATCHER, countriesMeta;
  var init_constants = __esm({
    "src/constants.ts"() {
      ...
      SPEEDTEST_URL = "http://www.gstatic.com/generate_204";
      PROXY_GROUPS = {
        SELECT: "选择代理",
        MANUAL: "手动选择",
        AUTO: "自动选择",
  ...
  function parseGroupType(args) {
    const fallback = args.loadbalance !== void 0 ? parseBool(args.loadbalance) ? 2 : 1 : 1;
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      landing ? {
        ...
        type: "select",
        ...regexFilter ? { "include-all": true, filter: LANDING_NODE_MATCHER.pattern } : { proxies: landingNodes.map((node) => node.name).filter(isNotNull) }
      } : null,
      {
      ...
      ...countryNames.map((country) => {
        ...
        if (!meta) return null;
        const nodeSource = !regexFilter ? { proxies: countryNodes[country]?.map((n) => n.name).filter(isNotNull) ?? [] } : {
          "include-all": true,
          filter: meta.pattern,
